...
function check_mode() {
    var p1;
    var p2;
    p1 = images.findMultiColors(captureScreen(), "#0095d6", [[1, 0, "#0095d6"], [2, 0, "#0095d6"]], { //蓝色开始按键
        region: [(device.height / 20) * 15, (device.width / 20) * 13],
        threshold: 1
    });
    sleep(100);
    p2 = images.findMultiColors(captureScreen(), "#1559be", [[1, 0, "#1559be"], [2, 0, "#1559be"]], { //联锁竞赛深蓝色开始按键
        region: [(device.height / 20) * 15, (device.width / 20) * 13],
        threshold: 1
    });
    sleep(100);
    if (p1 || p2) {
        sleep(200);
        set_window_status(0); // 开始作战时隐藏菜单
        if (p1) {
            mode = 'normal';
            window_header.time.setText("普通关卡");
        } else {
            mode = 'activity';
            window_header.time.setText("活动关卡");
        }
        sleep(1000);
    } else {
    ...
    for (var i = 1; i <= num; i++) {
        ...
        while (true) {
            ...
            if (p) {
                ...
                p = images.findMultiColors(captureScreen(), "#1559be", [[3, 0, "#1559be"], [5, 0, "#1559be"]], {
                    region: [(device.height / 20) * 15, (device.width / 20) * 13],
                    threshold: 1
                });
                sleep(100);
                if (p) {
                    ...
                    click(p.x + random(10, 50), p.y + random(1, 50));
                    sleep(1000);
                    p = images.findMultiColors(captureScreen(), "#1559be", [[3, 0, "#1559be"], [5, 0, "#1559be"]], { // 联锁竞赛深蓝色
                        region: [(device.height / 20) * 15, (device.width / 20) * 13],
                        threshold: 1
                    });
                    sleep(100);
